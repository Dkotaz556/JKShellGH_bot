...
#15GB wale gdrive account me upload karne ke liye
RUN echo "cHl0aG9uMyAuZnUvamtmdXAucHkgLWYgIiQxIg==" | base64 -d > /usr/local/bin/fup && chmod +x /usr/local/bin/fup
...
#change aria2c & ffmpeg name
RUN mv /usr/bin/aria2c /usr/bin/a
RUN mv /usr/bin/ffmpeg /usr/bin/f
